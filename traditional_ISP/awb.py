...
class AWB(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: Bayer изображение в формате RGGB, shape: [H, W], dtype: float32, range: [0, 0xFFFFFF]

        Returns:
            torch.Tensor: изображение с примененным AWB, shape: [H, W], dtype: float32, range: [0, 0xFFFFFF]
        """
