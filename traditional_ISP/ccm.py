...
class CCM(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: RGB изображение, shape: [H, W, 3], dtype: int32, range: [0, 0xFFFFFF]
        ...
        # Возвращаем форму и ограничиваем диапазон
        x_out = x_ccm.reshape(H, W, 3)
        x_out = torch.clamp(x_out, 0.0, 1.0)
        
        # Возвращаем float32 в диапазоне [0, 1] для следующих этапов
        return x_out
